...
class MissingModalityTransform(ImageOnlyTransform):
    def __init__(self, probability: RandomScalar = 0.2):
        ...
        with probability `p`. The segmentation remains unchanged.
    ...
    def get_parameters(self, **data_dict) -> dict:
        ...
        """
        dct = {}
        dct["mask_channel_0"] = torch.rand(1).item() < self.probability
        dct["mask_channel_1"] = torch.rand(1).item() < self.probability
        return dct
    ...
    def _apply_to_image(self, img: torch.Tensor, **params) -> torch.Tensor:
        ...
        """
        if params["mask_channel_0"]:
            img[0] = 0  # Set channel 0 to zero
        if params["mask_channel_1"]:
